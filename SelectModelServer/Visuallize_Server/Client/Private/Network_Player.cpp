...
CNetwork_Player::CNetwork_Player(CClient* pClient)
{
	m_pMemoryPooler = CMemoryPooler::GetInstance();
	m_pOwner = pClient;
}

CNetwork_Player::~CNetwork_Player()
{
	ClearUsedData();
...
void CNetwork_Player::SetTransform(const C2S_PACKET_PLAYER_TRANSFORM& tPacket)
{
...
void CNetwork_Player::SetInputs(const C2S_PACKET_PLAYER_INPUT& tPacket)
{
...
const S2C_PACKET_PLAYER_INPUT CNetwork_Player::GetInputs()
{
...
pair<int, CMemoryPooler::MemoryBlock*>& CNetwork_Player::GetLastDataBlock()
{
...
void CNetwork_Player::CalculateSendAdditionalPacket(char*& pOtherClientSendBuf, int& iOtherSendBufferSize)
{
	if (m_AdditionalDataList.empty())
		return;
...
void CNetwork_Player::ClearUsedData()
{
