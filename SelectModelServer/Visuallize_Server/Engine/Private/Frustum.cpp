...
void CFrustum::Tick()
{
	...
	_matrix			ViewMatrixInv = pPipeLine->GetTransformMatrixInverse(CPipeLine::D3DTS_VIEW);
	_matrix			ProjMatrixInv = pPipeLine->GetTransformMatrixInverse(CPipeLine::D3DTS_PROJ);
...
void CFrustum::TransformToLocalSpace(_fmatrix WorldMatrixInv)
{
	_vector			vPoints[8];
...
_bool CFrustum::isInWorldSpace(_fvector vWorldPos, _float fRadius)
{/*
	a b c d
	x y z 
...
_bool CFrustum::isInLocalSpace(_fvector vLocalPos, _float fRadius)
{
	for (_uint i = 0; i < 6; ++i)
	{
