...
HRESULT CNavigation::InitializePrototype(const _tchar * pNavigationDataFilePath)
{
	...
	if (FAILED(ReadyNeighbor()))
		return E_FAIL;

	// m_Cells[1]->SetNeighborIndex(CCell::LINE_AB, nullptr);
...
HRESULT CNavigation::Render()
{
	if (-1 == m_NavigationDesc.iCurrentIndex)
	{
		...
		m_Cells[m_NavigationDesc.iCurrentIndex]->RenderCell(0.05f, _float4(1.f, 0.f, 0.f, 1.f));
...
HRESULT CNavigation::ReadyNeighbor()
{
	for (auto& pSourCell : m_Cells)
	{
		for (auto& pDestCell : m_Cells)
		{
			...
			if (true == pDestCell->Compare(pSourCell->GetPoint(CCell::POINT_A), pSourCell->GetPoint(CCell::POINT_B)))
			{
				// 너(Dest)는 내(Sour) AB이웃. 
				pSourCell->SetNeighborIndex(CCell::LINE_AB, pDestCell);
			}

			if (true == pDestCell->Compare(pSourCell->GetPoint(CCell::POINT_B), pSourCell->GetPoint(CCell::POINT_C)))
			{				
				pSourCell->SetNeighborIndex(CCell::LINE_BC, pDestCell);
			}

			if (true == pDestCell->Compare(pSourCell->GetPoint(CCell::POINT_C), pSourCell->GetPoint(CCell::POINT_A)))
			{
				pSourCell->SetNeighborIndex(CCell::LINE_CA, pDestCell);
			}
		}
	}
...
CNavigation* CNavigation::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const _tchar * pNavigationDataFilePath)
{
	...
	if (FAILED(pInstance->InitializePrototype(pNavigationDataFilePath)))
	{
