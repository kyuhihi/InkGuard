...
#ifdef _DEBUG
HRESULT CCell::RenderCell(_float fHeight, _float4 vColor)
{
	...
	if (FAILED(m_pShader->SetRawValue("g_ViewMatrix", &pPipeLine->GetTransformFloat4x4TP(CPipeLine::D3DTS_VIEW), sizeof(_float4x4))))
		return E_FAIL;

	if (FAILED(m_pShader->SetRawValue("g_ProjMatrix", &pPipeLine->GetTransformFloat4x4TP(CPipeLine::D3DTS_PROJ), sizeof(_float4x4))))
		return E_FAIL;

	if (FAILED(m_pShader->SetRawValue("g_vColor", &vColor, sizeof(_float4))))
		return E_FAIL;
