...
void CQuadTree::Culling(const _float3* pVerticesPos, CFrustum* pFrustum, FACEINDICES32* pFaceIndices, _uint* pNumFaces)
{
		...
		true == LevelofDetail(pVerticesPos))
	{
		...
		_bool		isIn[] = {
			pFrustum->isInLocalSpace(XMLoadFloat3(&pVerticesPos[iIndices[0]])),
			pFrustum->isInLocalSpace(XMLoadFloat3(&pVerticesPos[iIndices[1]])),
			pFrustum->isInLocalSpace(XMLoadFloat3(&pVerticesPos[iIndices[2]])),
			pFrustum->isInLocalSpace(XMLoadFloat3(&pVerticesPos[iIndices[3]])),
		};
	...
	if (true == pFrustum->isInLocalSpace(XMLoadFloat3(&pVerticesPos[m_iCenterIndex]), fRadius))
	{
...
HRESULT CQuadTree::MakeNeighbor()
{
...
_bool CQuadTree::LevelofDetail(const _float3 * pVerticesPos)
{
	...
	_vector		vCamPosition = XMLoadFloat4(&m_pPipeLine->GetCamPosition());	
